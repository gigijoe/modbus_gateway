...
typedef struct {
    ...
    mb_param_type_t     mb_param_type;      /*!< Type of modbus parameter - function code */
...
mb_parameter_descriptor_t device_parameters[] = {
    { INDEX(0), 
        ...
        INPUT_OFFSET(fp0),
        PARAM_TYPE_FLOAT,
        4, /* Storage of registers data in bytes */
        OPTS( 0, 0, 0 ), /* Limition */
        PAR_PERMS_READ_WRITE_TRIGGER /* Access type */
    },
};
#endif

char *heap_strdup(const char *s) {
    if(s == NULL)
        return NULL;
    ...
    if(len <= 0)
        return NULL;
    char *r = (char *)heap_caps_malloc(len+1, MALLOC_CAP_SPIRAM);
    if(r == NULL)
        return NULL;
    memcpy(r, s, len+1);
    return r;
...
const mb_parameter_descriptor_t device_parameters[] = {
    ...
    { CID_INPUT_DATA_0, STR("Data_channel_0"), STR("Volts"), MB_DEVICE_ADDR1, MB_PARAM_INPUT, 0, 2,
                    INPUT_OFFSET(fp0), PARAM_TYPE_FLOAT, 4, OPTS( -10, 10, 1 ), PAR_PERMS_READ_WRITE_TRIGGER },
    { CID_HOLD_DATA_0, STR("Humidity_1"), STR("%rH"), MB_DEVICE_ADDR1, MB_PARAM_HOLDING, 0, 2,
            HOLD_OFFSET(fp0), PARAM_TYPE_FLOAT, 4, OPTS( 0, 100, 1 ), PAR_PERMS_READ_WRITE_TRIGGER },
    { CID_INPUT_DATA_1, STR("Temperature_1"), STR("C"), MB_DEVICE_ADDR1, MB_PARAM_INPUT, 2, 2,
            INPUT_OFFSET(fp1), PARAM_TYPE_FLOAT, 4, OPTS( -40, 100, 1 ), PAR_PERMS_READ_WRITE_TRIGGER },
    { CID_HOLD_DATA_1, STR("Humidity_2"), STR("%rH"), MB_DEVICE_ADDR1, MB_PARAM_HOLDING, 2, 2,
            HOLD_OFFSET(fp1), PARAM_TYPE_FLOAT, 4, OPTS( 0, 100, 1 ), PAR_PERMS_READ_WRITE_TRIGGER },
    { CID_INPUT_DATA_2, STR("Temperature_2"), STR("C"), MB_DEVICE_ADDR1, MB_PARAM_INPUT, 4, 2,
            INPUT_OFFSET(fp2), PARAM_TYPE_FLOAT, 4, OPTS( -40, 100, 1 ), PAR_PERMS_READ_WRITE_TRIGGER },
    { CID_HOLD_DATA_2, STR("Humidity_3"), STR("%rH"), MB_DEVICE_ADDR1, MB_PARAM_HOLDING, 4, 2,
            HOLD_OFFSET(fp2), PARAM_TYPE_FLOAT, 4, OPTS( 0, 100, 1 ), PAR_PERMS_READ_WRITE_TRIGGER },
/*
    ...
    { CID_RELAY_P1, STR("RelayP1"), STR("on/off"), MB_DEVICE_ADDR1, MB_PARAM_COIL, 0, 8,
            COIL_OFFSET(byte0), PARAM_TYPE_U16, 2, OPTS( BIT1, 0, 0 ), PAR_PERMS_READ_WRITE_TRIGGER },
*/            
    { CID_RELAY_P2, STR("RelayP2"), STR("on/off"), MB_DEVICE_ADDR1, MB_PARAM_COIL, 0, 8,
            COIL_OFFSET(byte0), PARAM_TYPE_U16, 2, OPTS( BIT0, 0, 0 ), PAR_PERMS_READ_WRITE_TRIGGER }
};

// Calculate number of parameters in the table
// const uint16_t num_device_parameters = (sizeof(device_parameters)/sizeof(device_parameters[0]));
static uint16_t num_device_parameters = 0;
...
static void master_operation_func(void *arg)
{
    ...
    ESP_LOGI(TAG, "Start modbus RTU master ...");
...
void mbRtuMasterTask(void *pvParameters) {
