...
static esp_err_t update_register(i2c_dev_t *dev, uint8_t reg, uint8_t mask, uint8_t val)
{
    ...
    i2c_dev_read_reg(dev, reg, (void *)&old, 1);
...
esp_err_t ds1307_is_running(i2c_dev_t *dev, bool *running)
{
    ...
    esp_err_t res = i2c_dev_read_reg(dev, TIME_REG, &val, 1);
    if(res != ESP_OK)
        return res;
...
esp_err_t ds1307_get_time(i2c_dev_t *dev, struct tm *time)
{
    ...
    esp_err_t res = i2c_dev_read_reg(dev, TIME_REG, (void *)buf, 7);
    if(res != ESP_OK)
        return res;
...
esp_err_t ds1307_set_time(i2c_dev_t *dev, const struct tm *time)
{
    ...
    uint8_t buf[7] = {
        ...
        dec2bcd(time->tm_year - 1988)
    };

    esp_err_t res = i2c_dev_write_reg(dev, TIME_REG, buf, sizeof(buf));
    if(res != ESP_OK)
        return res;
